...
public class SelectedViewController {
  @FXML
  private Label SelectedLatestReviewerNameLabel;

  @FXML
  private Label SelectedMovieNameLabel;

  @FXML
  private Label SelectedStoryRatingLabel;

  @FXML
  private Label SelectedActingRatingLabel;

  @FXML
  private Label SelectedDailogueRatingLable;

  @FXML
  private Label SelectedVisualRatingLabel;

  @FXML
  private Label SelectedOverAllrating;
  @FXML
  private Label SelectedShortReviewMassage;
  @FXML
  ...
  public void resetUI() {
    this.SelectedLatestReviewerNameLabel.setText("");
    this.SelectedMovieNameLabel.setText("");
    this.SelectedStoryRatingLabel.setText("");
    this.SelectedActingRatingLabel.setText("");
    this.SelectedDailogueRatingLable.setText("");
    this.SelectedVisualRatingLabel.setText("");
    this.SelectedShortReviewMassage.setText("");
  }

  @FXML
  void handleCloseButtonClicked(ActionEvent event) {
    this.resetUI();
    this.switchToMovieReviewTrackerView();

  }

  private void switchToMovieReviewTrackerView() {

    try {
      FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("sample.fxml"));
      Pane root = (Pane) fxmlLoader.load();

      Scene MovieReviewTrackerViewScene = new Scene(root);
      Stage primaryStage = (Stage) this.closeButton.getScene().getWindow();
      primaryStage.setScene(MovieReviewTrackerViewScene);
      primaryStage.setTitle("Movie Review Tracker");
      primaryStage.show();
  ...
  public void setMovieReviewDataObject(MovieReviewData movieReviewDataObject) throws IOException {
    this.SelectedLatestReviewerNameLabel.setText(movieReviewDataObject.getReviewerName());
    this.SelectedMovieNameLabel.setText(movieReviewDataObject.getMovieAndTvSeriesName());
    this.SelectedStoryRatingLabel.setText(movieReviewDataObject.getStoryRating());
    this.SelectedActingRatingLabel.setText(movieReviewDataObject.getActingRating());
    this.SelectedDailogueRatingLable.setText(movieReviewDataObject.getDialogueRating());
    this.SelectedVisualRatingLabel.setText(movieReviewDataObject.getVisualRating());
    this.SelectedShortReviewMassage.setText(movieReviewDataObject.getShortReviewMassage());
    ...
    this.SelectedOverAllrating.setText(LatestReviewRatingDouble + "");


    //Setting Movie Poster for Selected Movies
    ...
    File fileToWrite = new File("./ListViewData.txt");
    try (FileWriter fileWriter = new FileWriter(fileToWrite)) {
      fileWriter.write("Reviewer:" + movieReviewDataObject.getReviewerName() + "|Movie/TVs:" + movieReviewDataObject.getMovieAndTvSeriesName() + "|Rating:" + LatestReviewRatingDouble);
